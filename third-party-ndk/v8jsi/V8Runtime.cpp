...
v8::Isolate *V8Runtime::nodejsV8Isolate = nullptr;

V8Runtime::V8Runtime(
        std::unique_ptr<V8RuntimeConfig> config)
        : config_(std::move(config)) {
    assert(nodejsV8Isolate != nullptr);
    isolate_ = nodejsV8Isolate;
    ...
    LOG(INFO) << "V8Runtime constructor finish";
    return;
}
...
v8::Local<v8::Context> V8Runtime::CreateGlobalContext(v8::Isolate *isolate) {
    ...
    return scopedHandle.Escape(v8::Context::New(isolate_, nullptr, global));
}
...
const {
    ...
    std::string exception =
            JSIV8ValueConverter::ToSTLString(isolate, tryCatch->Message()->Get());
...
std::unique_ptr<v8::ScriptCompiler::Source> V8Runtime::UseFakeSourceIfNeeded(
    ...
    if (config_->codecacheMode == V8RuntimeConfig::CodecacheMode::kPrebuilt ||
        ...
        v8::Local<v8::String> stubScript =
                v8::String::NewFromUtf8(isolate_, stubScriptString.c_str()).ToLocalChecked();
